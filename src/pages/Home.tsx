...
function LeadForm() {
  ...
  const inputClass = "w-full bg-[var(--bg3)] border border-white/10 focus:border-[var(--accent)] outline-none px-4 py-3 text-white text-sm font-sans placeholder-gray-600 transition-colors duration-200";
  ...
  if (success) return (
    <div className="flex flex-col items-center justify-center py-12 text-center gap-4">
      ...
      <p className="text-xs font-sans" style={{ color: 'var(--text4)' }}>{t('lead_privacy')}</p>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <select value={form.budget} onChange={e => setForm({ ...form, budget: e.target.value })}
        className={selectClass} style={{ background: 'var(--bg3)' }} required>
        <option value="" className="bg-[var(--bg3)]">{t('lead_budget')}</option>
        <option value="<200k" className="bg-[var(--bg3)]">{t('lead_budget_1')}</option>
        <option value="200k-500k" className="bg-[var(--bg3)]">{t('lead_budget_2')}</option>
        <option value="500k-1m" className="bg-[var(--bg3)]">{t('lead_budget_3')}</option>
        <option value=">1m" className="bg-[var(--bg3)]">{t('lead_budget_4')}</option>
      </select>
      <select value={form.goal} onChange={e => setForm({ ...form, goal: e.target.value })}
        className={selectClass} style={{ background: 'var(--bg3)' }} required>
        <option value="" className="bg-[var(--bg3)]">{t('lead_goal_select')}</option>
        <option value="rental" className="bg-[var(--bg3)]">{t('lead_goal_1')}</option>
        <option value="offplan" className="bg-[var(--bg3)]">{t('lead_goal_2')}</option>
        <option value="visa" className="bg-[var(--bg3)]">{t('lead_goal_3')}</option>
        <option value="diversify" className="bg-[var(--bg3)]">{t('lead_goal_4')}</option>
      </select>
      ...
      <p className="text-center text-xs font-sans" style={{ color: 'var(--text4)' }}>{t('lead_privacy')}</p>
    </form>
  );
}
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-white" style={serif}>
...
<div className="relative z-10 w-full max-w-5xl mx-auto px-6 flex flex-col items-center text-center mt-24 md:mt-32">
  <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-[var(--accent)] text-xs font-sans font-light tracking-[0.4em] uppercase">{t('hero_tag')}</span>
            <div className="h-px w-12 bg-[var(--accent)]/50" />
          </div>

          <h1 className="text-5xl sm:text-7xl md:text-8xl font-light text-white leading-[0.9] tracking-tight mb-6">
            {t('hero_title_1')}<br />
            <em className="text-[var(--accent)] not-italic">{t('hero_title_em')}</em>
            <br />{t('hero_title_2')}
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mb-8">
            {['6–8% rendement', '0% impôt', 'Off-plan exclusif', 'Golden Visa'].map((tag) => (
              <span key={tag} className="text-xs font-sans px-3 py-1.5 tracking-wider"
                style={{ border: '1px solid rgba(251,191,36,0.25)', color: 'var(--accent)', backgroundColor: 'rgba(251,191,36,0.06)' }}>
                {tag}
              </span>
            ))}
          </div>

          <p className="text-[var(--text2)] text-sm sm:text-base max-w-lg mx-auto mb-10 leading-relaxed font-sans font-light">
            {t('hero_sub')}
          </p>
      ...
      {/* ── STATS ── */}
      <section className="border-y border-[var(--border2)] py-14 px-6">
        <div className="max-w-4xl mx-auto grid grid-cols-3 gap-8 text-center">
          {[
            ...
            <div key={i}>
              <div className="text-3xl sm:text-5xl font-light mb-2" style={{ color: 'var(--accent)' }}>
                <Counter target={s.value} suffix={s.suffix} />
              </div>
              <div className="text-xs font-sans tracking-[0.25em] uppercase" style={{ color: 'var(--text4)' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <div className="flex items-center justify-center gap-3 mb-6">
              ...
              <span className="text-[var(--accent)] text-xs font-sans tracking-[0.3em] uppercase">{t('why_invest_tag')}</span>
              <div className="h-px w-8 bg-[var(--accent)]" />
            </div>
            <h2 className="text-4xl sm:text-5xl font-light mb-4">
              {t('why_invest_title')} <em className="not-italic text-[var(--accent)]">{t('why_invest_em')}</em>
            </h2>
            <p className="text-[var(--text3)] font-sans font-light text-base max-w-xl mx-auto">{t('why_invest_sub')}</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px" style={{ backgroundColor: 'var(--border2)' }}>
            {[
              ...
              <div key={i} className="p-8 text-center transition-colors duration-300 group"
                ...
                <v.icon className="w-6 h-6 mx-auto mb-4 text-[var(--accent)]/50 group-hover:text-[var(--accent)] transition-colors" />
                <div className="text-base sm:text-lg font-light text-white mb-2">{v.val}</div>
                <p className="text-xs font-sans font-light leading-relaxed" style={{ color: 'var(--text4)' }}>{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── WHY MOVESMART ── */}
      <section className="py-28 px-6" style={{ backgroundColor: 'var(--bg2)' }}>
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          <div>
            <div className="flex items-center gap-3 mb-8">
              <div className="h-px w-8 bg-[var(--accent)]" />
              <span className="text-[var(--accent)] text-xs font-sans tracking-[0.3em] uppercase">{t('why_us_tag')}</span>
            </div>
            <h2 className="text-4xl sm:text-5xl font-light leading-tight mb-4">
              {t('why_us_title')}<br />
              <em className="not-italic text-[var(--accent)]">{t('why_us_em')}</em>
            </h2>
            <p className="text-xl font-light text-gray-300 mb-8 italic">{t('why_us_sub')}</p>
            <p className="text-[var(--text3)] leading-relaxed mb-5 font-sans font-light text-sm sm:text-base">{t('why_us_p1')}</p>
            <p className="text-[var(--text3)] leading-relaxed font-sans font-light text-sm sm:text-base mb-10">{t('why_us_p2')}</p>
            <Link to="/about"
              className="inline-flex items-center gap-3 text-[var(--accent)] text-xs font-sans tracking-[0.25em] uppercase hover:gap-5 transition-all duration-300">
      ...
      <section className="py-28 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <div className="flex items-center justify-center gap-3 mb-6">
              <div className="h-px w-8 bg-[var(--accent)]" />
              <span className="text-[var(--accent)] text-xs font-sans tracking-[0.3em] uppercase">{t('services_tag')}</span>
              <div className="h-px w-8 bg-[var(--accent)]" />
            </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px" style={{ backgroundColor: 'var(--border2)' }}>
            {[
              ...
              <div key={s.num} className="p-8 sm:p-10 transition-colors duration-300"
                ...
                <p className="text-xs sm:text-sm font-sans font-light leading-relaxed" style={{ color: 'var(--text4)' }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-16 gap-6">
            <div>
              <div className="flex items-center gap-3 mb-6">
                <div className="h-px w-8 bg-[var(--accent)]" />
                <span className="text-[var(--accent)] text-xs font-sans tracking-[0.3em] uppercase">{t('listings_tag')}</span>
              </div>
              <h2 className="text-4xl sm:text-5xl font-light">
                {t('listings_title_1')}<br />
                <em className="not-italic text-[var(--accent)]">{t('listings_title_em')}</em>
              </h2>
            </div>
            <Link to="/listings"
                ...
                <Link to={`/listings/${l.id}`} key={l.id} className="group block">
                  <div className="aspect-[4/3] overflow-hidden mb-5 relative" style={{ backgroundColor: 'rgba(255,255,255,0.03)' }}>
                    {l.images?.[0]
                      ...
                      : <div className="w-full h-full flex items-center justify-center text-xs font-sans" style={{ color: 'var(--text5)' }}>{t('listings_no_img')}</div>
                    }
                    <div className="absolute top-4 left-4 px-3 py-1" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }}>
                      <span className="text-[var(--accent)] text-xs font-sans tracking-widest uppercase">{l.type}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-sans mb-2" style={{ color: 'var(--text4)' }}>
                    <MapPin className="w-3 h-3" />{l.location}
                  </div>
                  <h3 className="text-xl font-light mb-3 group-hover:text-[var(--accent)] transition-colors duration-300">{l.title}</h3>
                  <div className="flex items-center gap-4 text-xs font-sans mb-4" style={{ color: 'var(--text4)' }}>
                  ...
                  <div className="flex items-center justify-between pt-4" style={{ borderTop: '1px solid var(--border)' }}>
                    <div className="text-2xl font-light" style={{ color: 'var(--accent)' }}>AED {l.price?.toLocaleString()}</div>
                    <span className="text-xs font-sans tracking-widest uppercase group-hover:text-[var(--accent)] transition-colors" style={{ color: 'var(--text4)' }}>{t('view_detail')}</span>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="py-28 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <div className="flex items-center justify-center gap-3 mb-6">
              <div className="h-px w-8 bg-[var(--accent)]" />
              <span className="text-[var(--accent)] text-xs font-sans tracking-[0.3em] uppercase">{t('roi_tag')}</span>
              <div className="h-px w-8 bg-[var(--accent)]" />
            </div>
            <h2 className="text-4xl sm:text-5xl font-light mb-4">
              {t('roi_title')} <em className="not-italic text-[var(--accent)]">{t('roi_em')}</em>
            </h2>
            <p className="text-[var(--text3)] font-sans font-light" >{t('roi_sub')}</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px mb-16" style={{ backgroundColor: 'var(--border2)' }}>
            {[
              ...
              <div key={i} className="p-8 text-center transition-colors duration-300"
                ...
                onMouseEnter={e => (e.currentTarget.style.backgroundColor = 'var(--bg3)')}
                onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'var(--bg)')}>
                <div className="text-4xl sm:text-5xl font-light mb-3" style={{ color: 'var(--accent)' }}>{r.val}</div>
                <div className="text-sm font-light mb-2 text-white">{r.label}</div>
                <div className="text-xs font-sans font-light" style={{ color: 'var(--text4)' }}>{r.desc}</div>
              </div>
            ))}
          </div>
          {/* Developer logos */}
          <div className="text-center">
            <div className="text-xs font-sans tracking-[0.3em] uppercase mb-8" style={{ color: 'var(--text5)' }}>{t('partners_label')}</div>
      ...
      {/* ── URGENCY + PROCESS ── */}
      <section className="py-20 px-6" style={{ backgroundColor: 'var(--bg2)' }}>
        <div className="max-w-6xl mx-auto">
          {/* Urgency banner */}
          <div className="mb-16 p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6"
            ...
            <div className="flex items-center gap-4">
              <div className="w-2.5 h-2.5 rounded-full bg-[var(--accent)] animate-pulse flex-shrink-0" />
              <div>
                <div className="text-xs font-sans tracking-[0.3em] uppercase text-[var(--accent)] mb-1">{t('urgency_badge')}</div>
          ...
          <div className="flex items-center gap-3 mb-10">
            <div className="h-px w-8 bg-[var(--accent)]" />
            <span className="text-[var(--accent)] text-xs font-sans tracking-[0.3em] uppercase">{t('process_tag')}</span>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <div key={p.step} className="relative">
                ...
                <p className="text-xs font-sans font-light leading-relaxed" style={{ color: 'var(--text4)' }}>{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-28 px-6">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            <div className="flex items-center justify-center gap-3 mb-6">
              <div className="h-px w-8 bg-[var(--accent)]" />
              <span className="text-[var(--accent)] text-xs font-sans tracking-[0.3em] uppercase">{t('lead_tag')}</span>
              <div className="h-px w-8 bg-[var(--accent)]" />
            </div>
            <h2 className="text-4xl sm:text-5xl font-light mb-4">
              {t('lead_title')}<br />
              <em className="not-italic text-[var(--accent)]">{t('lead_em')}</em>
            </h2>
            <p className="text-[var(--text3)] font-sans font-light text-base">{t('lead_sub')}</p>
          </div>
          <div className="p-8 sm:p-10" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--bg3)' }}>
            <LeadForm />
          </div>
        </div>
      </section>
      ...
      <section className="relative py-36 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          <div className="flex items-center gap-3 mb-8">
            <div className="h-px w-8 bg-[var(--accent)]" />
            <span className="text-[var(--accent)] text-xs font-sans tracking-[0.3em] uppercase">{t('cta_tag')}</span>
          </div>
          <h2 className="text-4xl sm:text-6xl lg:text-7xl font-light leading-[0.9] mb-10">
            {t('cta_title_1')}<br />
            {t('cta_title_2')} <em className="not-italic text-[var(--accent)]">{t('cta_title_em')}</em><br />
            {t('cta_title_3')}
          </h2>
          <p className="font-sans font-light text-base mb-12 max-w-md leading-relaxed" style={{ color: 'var(--text3)' }}>{t('cta_sub')}</p>
      ...
      {/* ── FOOTER ── */}
      <footer className="py-16 px-6" style={{ backgroundColor: 'var(--bg4)', borderTop: '1px solid var(--border2)' }}>
        <div className="max-w-6xl mx-auto">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
            <div className="lg:col-span-2">
              <div className="text-xl font-light text-white mb-4">MoveSmart<em className="not-italic text-[var(--accent)]"> Invest</em></div>
              <p className="text-xs font-sans font-light leading-relaxed mb-6 max-w-sm" style={{ color: 'var(--text4)' }}>
                {companyInfo.description}
              </p>
              <div className="flex items-center gap-2 text-xs font-sans" style={{ color: 'var(--text4)' }}>
                <MapPin className="w-3.5 h-3.5 text-[var(--accent)]" /> {companyInfo.location}
              </div>
            </div>
            <div>
              <div className="text-xs font-sans tracking-[0.3em] uppercase mb-6" style={{ color: 'var(--text4)' }}>{t('footer_nav')}</div>
              <ul className="space-y-3 text-sm font-sans" style={{ color: 'var(--text4)' }}>
                {[['/', t('home')], ['/listings', t('listings')], ['/about', t('about')], ['/contact', t('contact')]].map(([href, label]) => (
                  <li key={href}><Link to={href} className="hover:text-[var(--accent)] transition-colors">{label}</Link></li>
                ))}
              </ul>
            </div>
            <div>
              <div className="text-xs font-sans tracking-[0.3em] uppercase mb-6" style={{ color: 'var(--text4)' }}>{t('footer_contact')}</div>
              <ul className="space-y-3 text-sm font-sans" style={{ color: 'var(--text4)' }}>
                <li className="flex items-center gap-2"><Phone className="w-3.5 h-3.5 text-[var(--accent)]" />{companyInfo.whatsapp}</li>
                <li className="text-xs">contact@movesmartinvest.com</li>
              </ul>
            </div>
          </div>
          <div className="pt-8 flex flex-col sm:flex-row justify-between items-center gap-4" style={{ borderTop: '1px solid var(--border2)' }}>
            <p className="text-xs font-sans" style={{ color: 'var(--text5)' }}>© 2026 MoveSmart Invest. {t('footer_rights')}</p>
            <div className="flex gap-6 text-xs font-sans" style={{ color: 'var(--text5)' }}>
              <a href="#" className="hover:text-[var(--accent)] transition-colors">{t('footer_legal')}</a>
              <a href="#" className="hover:text-[var(--accent)] transition-colors">{t('footer_privacy')}</a>
            </div>
          </div>
        </div>
      </footer>
