...
import { LogOut, Plus, Edit2, Trash2, Eye, EyeOff, X, Save } from 'lucide-react';
        ...
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-2xl p-8">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition pr-10"
                    placeholder="••••••••"
                    required
                  />
              ...
              <button type="submit" disabled={loginLoading} className="w-full bg-amber-500 text-black py-3 rounded-lg font-semibold hover:bg-amber-400 transition disabled:opacity-50">
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-20">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        {successMsg && (
          <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg text-green-700 font-medium">{successMsg}</div>
        )}
                ...
                listings.map((l) => (
                  <tr key={l.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className="px-2 py-1 bg-amber-50 text-amber-700 rounded-full text-xs font-semibold">{l.type}</span>
                    </td>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })}
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none text-sm"
                  placeholder="Ex: Luxury Villa Downtown" required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none text-sm"
                    placeholder="Ex: Dubai Marina" required />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div>
                ...
                <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none text-sm"
                  placeholder="Description du bien..." rows={3} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">URLs des images</label>
                <p className="text-xs text-gray-400 mb-2">Une URL par ligne</p>
                <textarea value={form.images} onChange={e => setForm({ ...form, images: e.target.value })}
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none text-sm font-mono"
